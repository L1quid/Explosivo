...
void JNL_Connection::run(int max_send_bytes, int max_recv_bytes, int *bytes_sent, int *bytes_rcvd, int pollwait /* = -1*/)
{
  ...
  if(pollwait != -1 && m_socket > 0)
  {
	...
	tv.tv_usec = pollwait*1000;
    select(1,&f[0],&f[1],NULL,&tv);
  }
  switch (m_state)
  {
...
bool JNL_Connection::getbfromrecv(int pos, int remove, char *b)
{
	int read_pos=m_recv_pos-m_recv_len + pos;
	if (pos < 0 || pos > m_recv_len) return false;
	if (read_pos < 0)
	{
		read_pos += m_recv_buffer_len;
	}
...
int JNL_Connection::recv_line(char *line, int maxlength)
{
	...
	while (maxlength--)
	{
		...
		{
			char r;
			if (getbfromrecv(0,0, &r) && (r == '\r' || r == '\n') && r != t) getbfromrecv(0,1, &r);
			*line=0;
			return 0;
		}
		*line++=t;
