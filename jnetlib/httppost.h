...
class JNL_HTTPPost : public JNL_HTTPGet
{
  public:
	...
	int addfield(const char *name, const char *val);
	int addfile(const char *name, const char *mimetype, const char *filename, const char *filepath);
	int contentlength();
	unsigned long written(){return m_nwritten;};
	
protected:
	
	void _preparefield(const std::string &str);
