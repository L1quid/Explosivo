...
typedef enum
{
	JNL_IRC_USERSTATUS_NONE = 0,
	JNL_IRC_USERSTATUS_PRESENT,
	JNL_IRC_USERSTATUS_AWAY,
	JNL_IRC_USERSTATUS_OPER
}JNL_IRC_USERSTATUS;
class JNL_IRCParams
{
public:
...
public:
	...
	bool parse_LIST(std::string &channame, int &occupants, std::string &strmode, std::string &topicstring);
	bool parse_LISTEND(std::string &channame);
	bool parse_JOIN(std::string &shortname, std::string &fullname, std::string &channel);
	bool parse_PART(std::string &shortname, std::string &fullname, std::string &channel);
  bool parse_KICK(std::string &shortnameofkicker, std::string &channel, std::string &strwhokicked);
	bool parse_QUIT(std::string &shortname, std::string &strmessage);
	bool parse_NAMES(std::string &strchannel, std::map<std::string,int> &mchanusers);
	bool parse_TOPIC(std::string &shortname, std::string &fullname, std::string &channel, std::string &strtopic);
	bool parse_PRIVMSG(std::string &shortname, std::string &fullname, std::string &channel, std::string &strmsg, bool &bisaction);
	bool parse_NOSUCHNICK(std::string &shortname);
  bool parse_CHANNELJOINPROBLEM(std::string &channame, std::string &strreaason);
  bool parse_CHANNELISFULL(std::string &channame,std::string &strreason)
  {
    return parse_CHANNELJOINPROBLEM(channame,strreason);
  }
  bool parse_INVITEONLYCHANNEL(std::string &channame,std::string &strreason)
  {
    return parse_CHANNELJOINPROBLEM(channame,strreason);
  }
  bool parse_BANNEDFROMCHANNEL(std::string &channame,std::string &strreason)
  {
    return parse_CHANNELJOINPROBLEM(channame,strreason);
  }
  bool parse_BADCHANNELKEY(std::string &channame,std::string &strreason)
  {
    return parse_CHANNELJOINPROBLEM(channame,strreason);
  }
...
public:
	...
	bool connect(char *szHost, unsigned short nPort=6667, int insz=100*1024, int outsz=100*1024);
	void close()
	{
		m_con->close(1);
	}
	bool send_message(const char *fmt,...);
	bool run(int pollwait=-1);
  int  get_state()
  {
    if(m_con)
    {
    ...
    {
      return JNL_Connection::STATE_CLOSED;
    }
  }
...
public:
	...
	int get_state()
  {
	...
	bool connect(char *szHost, unsigned short nPort=6667, int insz=100*1024, int outsz=100*1024)
	{
		return m_con.connect(szHost,nPort,insz,outsz);
	}
	bool run(int pollwait=-1);

	virtual JNL_IRC_RETVAL process_message(JNL_IRCMessage *pm, bool deletemessage=true);

	virtual bool sendpass(char *password);
	virtual bool nick(char *nick);
	virtual bool oper(char *user, char *password);
	virtual bool quit(char *swansong);
	virtual bool part(char *swansong);
	virtual bool join(char *channel, char *key=NULL);
	virtual bool setmode(char *channel, int isadd, char mode, char *param=NULL);
	virtual bool topic(char *channel, char *topic=NULL);
	virtual bool userregistration(char *username="poopoo", char *hostname="hostname", char *servername="servername", char *realname="poopoo platter");
	virtual bool opuser(char *channel, char *nick)
	{
		return setmode(channel,1,'o',nick);
	}
	virtual bool deopuser(char *channel, char *nick)
	{
		return setmode(channel,0,'o',nick);
	}
	virtual bool voiceuser(char *channel, char *nick)
	{
		return setmode(channel,1,'v',nick);
	}
	virtual bool devoiceuser(char *channel, char *nick)
	{
		return setmode(channel,0,'v',nick);
	}
	virtual bool banuser(char *channel, char *banstring)
	{
		return setmode(channel,1,'b',banstring);
	}
	virtual bool unbanuser(char *channel, char *banstring)
	{
		return setmode(channel,0,'b',banstring);
	}
	virtual bool names(char *channel=NULL);
	virtual bool list(char *channel=NULL);
	virtual bool invite(char *nick, char *channel);
	virtual bool kick(char *channel, char *user, char *swansong=NULL);
	virtual bool version(char *server=NULL);
	virtual bool stats(char query = 0, char *server=NULL);
	virtual bool links(char *remote=NULL, char *mask=NULL);
	virtual bool time(char *server=NULL);
	/*
	**  Missing trace and connect
	*/
	virtual bool admin(char *server=NULL);
	virtual bool info(char *server=NULL);
	virtual bool privmsg(char *recipient, char *msg, bool baction=false);
	virtual bool notice(char *recipient, char *msg);
	virtual bool who(char *nick, char o=0);
	virtual bool whois(char *server,char *nick);
	virtual bool ping(char *server);
	bool pong(char *server);

	int privmsgbytesover(const char *szchan, const char *txt, bool baction);
	int maxprivmsgbytes(const char *szchan);
  int baseprivmsgbytesneeded();
...
protected:
	...
	*/
	...
	virtual JNL_IRC_RETVAL onRplUserMe(JNL_IRCMessage &m){m.messagenum();return JNL_IRC_RETVAL_OK;};		
	/*
